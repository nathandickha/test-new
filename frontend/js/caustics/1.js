...
export class CausticsSystem {
  ...
  applyToMaterial(mat) {
    ...
    mat.onBeforeCompile = (shader) => {
      ...
      // WaterSim height texture (optional)
      const _dummyTex = new THREE.DataTexture(new Uint8Array([0,0,0,255]), 1, 1);
      _dummyTex.needsUpdate = true;
      shader.uniforms.uWaterHeightTex      = { value: this.waterHeightTex || _dummyTex };
      shader.uniforms.uWaterHeightTexValid = { value: this.waterHeightTex ? 1.0 : 0.0 };
      shader.uniforms.uWaterTexel          = { value: this.waterTexel.clone() };

      // RGB dispersion amount (small, subtle)
      shader.uniforms.uCausticsChroma      = { value: 0.018 };
...
if (uCausticsEnabled > 0.5) {
  ...
  // --- RGB dispersed caustics (subtle chroma like the reference) ---
  // Offsets are in "projected UV" units (after worldToUv + scaling), so keep tiny.
  float chroma = uCausticsChroma;

  vec2 ox = vec2( 1.0, -0.6) * chroma;
  vec2 oy = vec2(-0.7,  1.0) * chroma;
  vec2 oz = vec2( 0.8,  0.5) * chroma;

  // Procedural caustics per-channel
  float cxR = caustics(px + ox);
  float cxG = caustics(px);
  float cxB = caustics(px - ox);

  float cyR = caustics(py + oy);
  float cyG = caustics(py);
  float cyB = caustics(py - oy);

  float czR = caustics(pz + oz);
  float czG = caustics(pz);
  float czB = caustics(pz - oz);

  vec3 cProc = vec3(
    cxR*an.x + cyR*an.y + czR*an.z,
    cxG*an.x + cyG*an.y + czG*an.z,
    cxB*an.x + cyB*an.y + czB*an.z
  );

  // Optional: blend in heightmap-driven caustics if WaterSim is available
  if (uWaterHeightTexValid > 0.5) {
    float hx = heightCaustics(px);
    float hy = heightCaustics(py);
    float hz = heightCaustics(pz);
    float h  = hx*an.x + hy*an.y + hz*an.z;

    // Inject as additional "energy" (keeps the web look but adds natural flicker)
    cProc += vec3(h) * 0.55;
  }

  // Shape for readability and softer "shadow-like" falloff
  cProc = smoothstep(vec3(0.03), vec3(0.92), cProc);
  cProc = pow(clamp(cProc, 0.0, 1.0), vec3(1.35));

  // Directional bias (keep it subtle so it doesn't disappear)
  float ndl = clamp(dot(normalize(normal), normalize(uSunDir)), 0.25, 1.0);

  vec3 caRGB = cProc * (uCausticsStrength * pow(ndl, 0.6));

  // Projected-light look: add light energy onto albedo
  vec3 tint = vec3(0.90, 0.97, 1.0);
  gl_FragColor.rgb *= (1.0 + tint * caRGB);
}
`
);
  ...
  _syncUniforms(mat, force = false) {
    ...
    // WaterSim (optional)
    if (shader.uniforms.uWaterHeightTex) {
      shader.uniforms.uWaterHeightTex.value = this.waterHeightTex || shader.uniforms.uWaterHeightTex.value;
    }
    if (shader.uniforms.uWaterHeightTexValid) {
      shader.uniforms.uWaterHeightTexValid.value = this.waterHeightTex ? 1.0 : 0.0;
    }
    if (shader.uniforms.uWaterTexel) {
      shader.uniforms.uWaterTexel.value.copy(this.waterTexel);
    }
}
}
